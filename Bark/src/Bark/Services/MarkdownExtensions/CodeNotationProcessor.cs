...
/// <summary>
/// Result of processing a fenced code block's raw lines for vitepress/shiki line-notation
/// comments (<c>[!code highlight]</c>, <c>[!code ++]</c>, <c>[!code word:x]</c>, etc.) and meta
/// highlight ranges.
/// </summary>
...
/// <summary>
/// Ports the comment-notation parsing from @shikijs/transformers (notation-highlight.ts,
/// notation-diff.ts, notation-focus.ts, notation-error-level.ts, notation-highlight-word.ts) as
/// used by vitepress's highlight.ts, operating on raw source text instead of a tokenized AST
/// since per-line marker stripping has to happen before tokenization either way.
/// </summary>
public static class CodeNotationProcessor
{
    ...
    [
        ...
        // vitepress overrides shiki's notation-focus defaults in node/markdown/plugins/highlight.ts.
    ...
    // @shikijs/transformers notation-highlight-word.ts: RE_CODE_WORD =
    // /\s*\[!code word:((?:\\.|[^:\]])+)(:\d+)?\]/
    ...
    private static Regex BuildCommentMarkerRegex(Func<string, string> innerPattern, string namesArg)
    {
        ...
        // Mirrors @shikijs/transformers parse-comments.ts `matchers`: block comments anchored
        // whole-line, line comments (//, #, ;, ;;, %, %%, --) anchored at end of line. The same
        // named groups are reused across alternatives -- .NET regex allows this since only one
        // alternative can match for a given input.
        var pattern =
            $@"^(?<pre>.*?)(?:" +
            $@"<!--\s*\[!code\s+{inner}\]\s*-->|" +
            $@"/\*\s*\[!code\s+{inner}\]\s*\*/|" +
