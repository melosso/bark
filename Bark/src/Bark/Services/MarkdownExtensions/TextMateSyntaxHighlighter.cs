...
/// <summary>
/// Grammar-based <see cref="ISyntaxHighlighter"/> backed by TextMateSharp (the same vscode-textmate
/// engine family Shiki itself wraps), tokenizing against vitepress's default github-light/dark
/// theme pair so colors match vitepress output exactly. Dual-theme: each line is tokenized once
/// (scopes are theme-independent) and resolved to a light and a dark color per token, emitted as
/// <c>--shiki-light</c>/<c>--shiki-dark</c> CSS vars -- the same mechanism Shiki/vitepress use, so
/// Bark's existing `[data-theme]` light/dark toggle drives token color for free.
/// </summary>
public sealed class TextMateSyntaxHighlighter : ISyntaxHighlighter
{
    ...
    // vitepress's transformerDisableShellSymbolSelect: prevents the leading "$"/">" shell
    // prompt symbol from being selected/copied along with the command.
