...
/// <summary>
/// Parsed vitepress-style fenced code block meta: language, tab title, line-numbers toggle,
/// and `{1,3-5}` highlighted line ranges. Mirrors vitepress's preWrapper.ts / highlight.ts /
/// @shikijs/transformers meta-highlight parsing.
/// </summary>
...
{
    // Mirrors vitepress preWrapper.ts#extractLang / highlight.ts langRE: keep in sync.
    private static readonly Regex LangRegex = new(@"^[a-zA-Z0-9_-]+", RegexOptions.Compiled);

    // vitepress preWrapper.ts: extractTitle via /\[(.*)\]/
    ...
    // @shikijs/transformers meta-highlight.ts: RE_HIGHLIGHT_LINES = /\{([\d,-]+)\}/
    private static readonly Regex HighlightRangeRegex = new(@"\{([\d,-]+)\}", RegexOptions.Compiled);

    // @shikijs/transformers meta-highlight-word.ts: RE_WORD_MATCH = /\/((?:\\.|[^/])+)\//g
    ...
    /// <summary>
    /// Parses a Markdig fenced code block's <c>Info</c> (lang token) and <c>Arguments</c> (rest of
    /// the info-string line) back into a single combined string, exactly as vitepress/markdown-it
    /// would have seen it (they never split the info string), then applies vitepress's parsing rules.
    /// </summary>
    ...
    private static string ExtractLang(string combined)
    {
        ...
        // Keep in sync with vitepress preWrapper.ts#extractLang / highlight.ts vueRE.
