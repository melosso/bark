...
/// <summary>
/// Replaces Markdig's default <see cref="CodeBlockRenderer"/> with vitepress-compatible markup:
/// the `language-{lang}` wrapper div with copy button + lang label (preWrapper.ts), per-line
/// `&lt;span class="line"&gt;` wrapping with highlight/diff/focus/error/warning notation classes
/// (highlight.ts via @shikijs/transformers), and an optional line-numbers gutter (lineNumbers.ts).
///
/// Bark has no language-aware tokenizer (no Shiki equivalent), so output lines are HTML-escaped
/// as plain text rather than colorized spans-per-token. CSS classes match vitepress 1:1 so
/// vitepress themes/snippets that target `.line.highlighted` etc. still apply correctly.
/// </summary>
