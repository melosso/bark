...
public sealed class DocumentationService : IHostedService, IDisposable
{
    ...
    private async Task BuildAsync(CancellationToken cancellationToken)
    {
        ...
        // Loaded up front (not after the page loop) so each page's title fallback can consult
        // config.json's nav-configured title before falling back to the filename.
        var config = LoadConfig(docsPath);
        var navTitlesByPath = BuildNavTitleLookup(config);

        // Sorted for deterministic hashing below, regardless of filesystem enumeration order.
        ...
        // FileSystemWatcher (inotify) is unreliable over non-native filesystems (network shares,
        // NTFS/exFAT, FUSE mounts), and can fire spurious change events with no actual content
        // change. Only bump BuildVersion -- which the client polls and reloads on -- when the
        // rebuilt content actually differs, so spurious events don't cause constant client reloads.
