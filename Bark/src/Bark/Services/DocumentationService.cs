...
public sealed class DocumentationService : IHostedService, IDisposable
{
    ...
    private async Task BuildAsync(CancellationToken cancellationToken)
    {
        ...
        var allFiles = Directory.GetFiles(docsPath, "*.md", SearchOption.AllDirectories);
        ...
        foreach (var file in allFiles)
        {
            ...
            var (html, title, description, headings) = _markdown.Parse(content, defaultTitle);
        ...
        _docsConfig = LoadConfig(docsPath);
    ...
    {
        var configPath = Path.Combine(docsPath, "bark.json");
