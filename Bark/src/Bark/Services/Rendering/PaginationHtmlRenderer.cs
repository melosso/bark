...
public static class PaginationHtmlRenderer
{
    public static string BuildPaginationHtml(string? prevTitle, string? prevPath, string? nextTitle, string? nextPath, string basePath)
    {
        ...
        if (prevPath != null)
        {
            ...
            html.AppendLine($"<span class=\"label\">{LayoutProvider.HtmlEncode(l.PaginationPrevious)}</span>");
        ...
        if (nextPath != null)
        {
            ...
            html.AppendLine($"<span class=\"label\">{LayoutProvider.HtmlEncode(l.PaginationNext)}</span>");
