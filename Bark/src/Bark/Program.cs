...
try
{
    ...
    // Grammar/theme loading is one-time and synchronous-ish (file I/O), so it must finish before
    // DocumentationService's hosted-service StartAsync (which renders every page) begins.
    ...
    {
        // Must never be cached -- the hot-reload poll relies on always seeing the live value,
        // not a browser-cached one (a "no-cache" page header still allows heuristic caching for
        // a small JSON endpoint with no validators, so this needs the stronger "no-store").
    ...
    app.MapGet("/{**path}", async (string? path, DocumentationService docs, MarkdownService markdown, HttpContext context) =>
    {
        ...
        // Home pages (layout: home) never get prev/next pagination, same as vitepress's default theme.
        ...
        var editLinkHtml = !isHomePage && config?.EditLink is { Pattern: { Length: > 0 } pattern } editLink
            ? $"<a class=\"edit-link\" href=\"{LayoutProvider.HtmlEncode(pattern.Replace(":path", $"{page.Path}.md"))}\" " +
              $"target=\"_blank\" rel=\"noopener noreferrer\">{LayoutProvider.HtmlEncode(editLink.Text)}</a>"
            : string.Empty;
...
static void AppendSidebarEntry(StringBuilder html, NavEntry entry, string currentPath, int level)
{
    ...
    // browsers apply enough UA-default styling to <summary> that no class override fully
    // neutralizes it, so the collapsible and static variants would render inconsistently.
    // <summary> only ever wraps that div as a near-invisible click target.
    if (isCollapsible)
