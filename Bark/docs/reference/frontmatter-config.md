...
> [!NOTE]  
> Bark doesn't currently support per-page `prev`/`next` overrides or a `sidebar: false` toggle the way vitepress does. Pagination always derives from your nav order, and every page that resolves to a `.md` file gets a sidebar. See [Prev / Next Links](default-theme-prev-next-links) for the workaround.
