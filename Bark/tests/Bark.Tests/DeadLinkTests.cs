...
public sealed partial class DeadLinkTests : IDisposable
{
    ...
    {
        _tempDir = Path.Combine(Path.GetTempPath(), "bark-deadlink-" + Guid.NewGuid().ToString("N"));
    ...
    private DocumentationService CreateService(string? configJson = null)
    {
        if (configJson != null)
        ...
        return new DocumentationService(options, _markdown, NullLogger<DocumentationService>.Instance);
    }
    ...
    public async Task RunOnRealDocs_NoDeadInternalLinks()
    {
        ...
        _service = new DocumentationService(options, _markdown, NullLogger<DocumentationService>.Instance);
        await _service.StartAsync(CancellationToken.None);
